"""
Localhost-only API for Fincra Naira checkout tests.
Dummy login + in-memory wallet. Do not deploy this file.
"""
...
os.environ.setdefault(
    "JWT_SECRET_KEY",
    "calliotel-local-fincra-dev-secret-key-32c",
)
...
from routes import fincra_routes

app = FastAPI(title="Calliotel local Fincra")
app.add_middleware(
    CORSMiddleware,
    allow_origins=[
        "http://localhost:3000",
...
app.include_router(fincra_routes.router, prefix="/api/payments/fincra", tags=["Fincra Naira"])
...
@app.get("/api/health")
async def health():
    return {"status": "alive", "service": "calliotel-local-fincra", "demo": True}
...
if __name__ == "__main__":
    import uvicorn
    uvicorn.run("local_fincra_app:app", host="127.0.0.1", port=8000, reload=True)
